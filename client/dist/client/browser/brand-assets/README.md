...
| [`ptc-safe-ride-side.png`](ptc-safe-ride-side.png) / [`ptc-safe-ride-back.png`](ptc-safe-ride-back.png) | **Safe Ride van cutouts** (transparent, ~300 px wide) — the van alone, background removed, used by the home-page "Coming Soon" banner. Generated from the `ptsf_*` sources by [`scripts/cutout-art.mjs`](../scripts/cutout-art.mjs) (exact commands are in that file's header); regenerate rather than hand-editing. |
